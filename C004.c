...
int applyMyClasses(int my[], int msize, struct st_class* c[], int csize) {
    ...
    do {
        ...
        if (!found) {
            printf("Invalid class code. Please try again.\n");
            continue;
        }

        // Add the class code to the list of applied classes
        if (msize < 10) {
            my[msize] = code;
            msize++;
            printf("Class with code %d applied.\n", code);
        } else {
            printf("Maximum number of classes reached (10 classes).\n");
            break;
        }

        printf("Add more classes? (1: Yes, 2: No) > ");
        scanf("%d", &choice);
    } while (choice == 1 && msize < 10);
